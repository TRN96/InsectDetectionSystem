...
def visualize_boxes_and_labels_on_image_array(
  ...
  """
    ...
    db_path = os.path.join(BASE_DIR, "IDS_DB.db")
    if how_many_object == {}:
        ...
        for x in how_many_object:
            ...
            c.execute('SELECT * FROM insects WHERE LOWER(insect_name) LIKE?', (x,))
            insect = c.fetchall()

            if insect:
                insectName = format(insect[0][1], "<25")
                insectCount = format(how_many_object[x], "^15")
                PesticideName = format(insect[0][3], ">25")

                resultList.append(str(insectName + insectCount + PesticideName))
                result.set('\n'.join(resultList))
                print(resultList)
            else:
                result.set('No insect detected')
            ...
            outputFrame.update()
        # c.execute('SELECT insect_id FROM insects WHERE insect_name=?', (x,))
        # insect_id = c.fetchall()
        # print(insect_id)
        # c.execute('SELECT * FROM pesticides WHERE pesticide_id=?', (insect_id,))
        # pesticide = c.fetchall()

        # print ('The pesticide recommended is ', pesticide)

    return image


'''def read_from_db (class_name):
    ''''''conn = sqlite3.connect('IDS_DB.db')
    c = conn.cursor()''''''
    ...
    c.execute('SELECT * FROM insects WHERE insect_name=?', (variable))
    insect = c.fetchall()

    c.execute('SELECT * FROM pesticides WHERE pesticide_id=?', (variable))
    pesticide = c.fetchall()
    
    print ('The insect detected is ', insect)
    print ('The pesticide recconmended is ', pesticide)
